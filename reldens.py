...
def reldens(nrad, rad, stdrho, ntm1, dnt, r, hob, dr2, rho, drrho, dr, ndr, nt, rhoh, drrhoh) :
	...
	while (j < ntm1) :
		...
		h1 = r[0] * cost + hob
		h2 = (r[0] - dr2) * cost + hob 
		rho[0][j] = math.exp(TABLIN(rad, 2, nrad, h1, 1))
		rhod = math.exp(TABLIN(rad, 2, nrad, h2, 1))
		drrho[0][j] = rhod * dr
		i = 1
		while (i < ndr) :
			h1 = r[i] * cost + hob
			h2 = (r[i] - dr2) * cost + hob 
			rho[i][j] = math.exp(TABLIN(rad, 2, nrad, h1, 1))
			rhod = math.exp(TABLIN(rad, 2, nrad, h2, 1))
			drrho[i][j] = rhod * dr + drrho[i-1][j]
			i += 1 #20
		j += 1 #30
	...
	while (j < nt) :
		...
		h1 = r[0] * cost + hob
		h2 = (r[0] – dr2) * cost + hob
		rhoh[0][j] = math.exp(TABLIN(rad, 2, nrad, h1, 1))
		rhod = math.exp(TABLIN(rad, 2, nrad, h2, 1))
		drrhoh[0][j] = rhod * dr
		i = 1
		while (i < ndr) :
			h1 = r[i] * cost + hob
			h2 = (r[i] – dr2) * cost + hob
			rhoh[i][j] = math.exp(TABLIN(rad, 2, nrad, h1, 1))
			rhod = math.exp(TABLIN(rad, 2, nrad, h1, 1))
			drrhoh[i][j] = rhod * dr + drrhoh[i-1][j]
			i += 1 #40
		j += 1 #50
